...
class CustomerDetailsActivity : AppCompatActivity() {
    ...
    private fun setViews() {
        ...
        if (customer.value?.phoneNumber.isNullOrEmpty() || customer.value?.phoneNumber == "null") {
            ...
            customer.value?.phoneNumber?.let {
...
//                packageManager.getPackageInfo(packageName, PackageManager.GET_ACTIVITIES)
    ...
    private fun makePhoneCall() {
        val callURI = Uri.parse(
            ConstantsVariables.calURIPrefix + ConstantsVariables.phoneIndex +
...
//            intent.getParcelableExtra(ConstantsVariables.customerKey) as? Customer
        ...
        // Should remove this if we target API 33
        return intent.getParcelableExtra(ConstantsVariables.customerKey) as? Customer
    }
